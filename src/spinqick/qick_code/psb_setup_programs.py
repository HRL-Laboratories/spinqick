...
class PSBScanGeneral(averager_program.NDAveragerProgram, readout.Readout):
    ...
    def body(self):
        ...
        if scan_type == "meas":
            ### for executing this with no ramp. This worked for us for testing, but inthe future we really want a ramp here.
        ...
        self.measure(
            ...
            t=0,
            wait=True,
            syncdelay=self.cfg.PSB_cfg.times.settle_time,
        )
        ...
        self.pulse(ch=Px_gen, t=0)
        self.pulse(ch=Py_gen, t=0)
        self.sync_all(self.cfg.PSB_cfg.times.idle_time)
        ...
        if scan_type == "meas":
            ...
            # if initialization is involving the x-gate, pulse x at entry point and then set to zero
            if self.cfg.PSB_sweep_cfg.x_init is True:
                self.set_pulse_registers(
                    ch=X_gen, waveform="baseband", gain=X_cfg.gains.entry2_gain
                )
        ...
        if scan_type in ["flush", "flush_2", "idle"]:
            if self.cfg.PSB_sweep_cfg.x_init is True:
                self.set_pulse_registers(
                    ch=X_gen, waveform="baseband", gain=Defaults.GAIN
                )
                self.pulse(ch=X_gen, t=0)
            for Pgen in [Px_gen, Py_gen]:
