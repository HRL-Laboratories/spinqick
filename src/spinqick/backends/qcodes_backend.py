...
class QCoDesHandler(DataHandler):
    ...
    :param db_path: Path to the SQLite database file.  Created if it does not
        exist.  Defaults to ``"<data_directory>/<db_name>"``.
    :param db_name: Name of the SQLite database file.  Defaults to ``"spinqick.db"``.
    ...
    def __init__(
        ...
        if db_path is None:
            ...
            db_path = file_settings.data_directory

        full_path = os.path.join(db_path, db_name) if db_name is not None else db_path
        self.db_path = os.path.realpath(full_path)
